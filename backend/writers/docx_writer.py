...
_EQUATION_PLACEHOLDER_RE = re.compile(r"\[EQUATION_(\d+)\]")
...
def _has_ancestor(element: Any, stop: Any, tag: str) -> bool:
...
def _is_math_element(element: Any) -> bool:
    return element.tag in _MATH_TAGS


def _visible_text_from_element(element: Any, stop: Any) -> str:
    parts: list[str] = []
    for node in element.iter():
        if _has_ancestor(node, stop, qn('w:del')):
            continue
        if node.tag == qn('w:t'):
...
def _math_aware_text(para: Any) -> str:
    parts: list[str] = []
    equation_index = 0
    for child in para._p:
        if _is_math_element(child):
            equation_index += 1
            parts.append(f'[EQUATION_{equation_index}]')
            continue
        if child.tag == qn('w:del'):
            continue
        parts.append(_visible_text_from_element(child, para._p))
    return ''.join(parts)


def _runs_from_layout_child(child: Any, para: Any) -> list[Run]:
    if child.tag == qn('w:r'):
        return [Run(child, para)]
    if child.tag in (qn('w:hyperlink'), qn('w:ins')):
        return [
            Run(r, para)
            for r in child.findall(qn('w:r'))
            if not _has_ancestor(r, para._p, qn('w:del'))
        ]
    return []


def _paragraph_math_layout(para: Any) -> tuple[list[dict[str, Any]], int]:
    intervals: list[dict[str, Any]] = [{"runs": [], "after": None, "before": None}]
    math_count = 0
    for child in para._p:
        if _is_math_element(child):
            math_count += 1
            intervals[-1]["before"] = child
            intervals.append({"runs": [], "after": child, "before": None})
            continue
        if child.tag == qn('w:del'):
            continue
        intervals[-1]["runs"].extend(_runs_from_layout_child(child, para))
    return intervals, math_count


def _split_text_by_placeholders(text: str, math_count: int) -> list[str] | None:
    segments: list[str] = []
    cursor = 0
    expected = 1
    for match in _EQUATION_PLACEHOLDER_RE.finditer(text):
        equation_index = int(match.group(1))
        if equation_index != expected:
            continue
        segments.append(text[cursor:match.start()])
        cursor = match.end()
        expected += 1
        if expected > math_count:
            break

    if expected != math_count + 1:
        return None
...
def _nearest_split(text: str, target: int) -> int:
    if target <= 0:
        return 0
    if target >= len(text):
        return len(text)

    search_radius = min(30, max(target, len(text) - target))
    for offset in range(search_radius + 1):
        for pos in (target + offset, target - offset):
            if 0 < pos < len(text) and text[pos - 1].isspace():
                return pos
...
def _split_text_by_original_layout(text: str, intervals: list[dict[str, Any]]) -> list[str]:
    original_segments = [
        ''.join(run.text for run in interval["runs"])
        for interval in intervals
    ]
    non_empty = [i for i, segment in enumerate(original_segments) if segment.strip()]
    if not non_empty:
        return [text] + [""] * (len(intervals) - 1)
    if len(non_empty) == 1:
        segments = [""] * len(intervals)
        segments[non_empty[0]] = text
        return segments

    lines = text.splitlines()
    if len(lines) == len(non_empty):
        segments = [""] * len(intervals)
        for idx, line in zip(non_empty, lines):
            segments[idx] = line
        return segments

    total_original = sum(max(len(original_segments[idx].strip()), 1) for idx in non_empty)
    segments = [""] * len(intervals)
    cursor = 0
    used_original = 0
    for idx in non_empty[:-1]:
        used_original += max(len(original_segments[idx].strip()), 1)
        target = round(len(text) * used_original / total_original)
        split_at = _nearest_split(text, target)
        segments[idx] = text[cursor:split_at].strip()
        cursor = split_at
    segments[non_empty[-1]] = text[cursor:].strip()
    return segments


def _ensure_interval_run(para: Any, interval: dict[str, Any]) -> Run:
    if interval["runs"]:
        return interval["runs"][0]

    run_el = OxmlElement('w:r')
    before = interval.get("before")
    after = interval.get("after")
    if before is not None:
        before.addprevious(run_el)
    elif after is not None:
        after.addnext(run_el)
    else:
        para._p.append(run_el)

    run = Run(run_el, para)
    interval["runs"].append(run)
    return run


def _apply_run_formatting(run: Run, font_name: str | None = None, font_size: float | None = None,
...
def _set_math_aware_paragraph_text(para: Any, text: str, font_name: str | None = None, font_size: float | None = None,
                                   align: str | None = None, bold: bool | None = None, italic: bool | None = None,
                                   underline: bool | None = None, strike: bool | None = None, bullet: bool | None = None) -> bool:
    intervals, math_count = _paragraph_math_layout(para)
    if math_count == 0:
        return False

    original_text = _math_aware_text(para)
    if text.strip() == original_text.strip():
        segments = _split_text_by_placeholders(original_text, math_count) or _split_text_by_original_layout("", intervals)
    else:
        segments = _split_text_by_placeholders(text, math_count)
        if segments is None:
            segments = _split_text_by_original_layout(_EQUATION_PLACEHOLDER_RE.sub("", text).strip(), intervals)

    for interval in intervals:
        for run in interval["runs"]:
            run.text = ""

    for interval, segment in zip(intervals, segments):
        if not segment:
            continue
        run = _ensure_interval_run(para, interval)
        run.text = segment
    ...
    for interval in intervals:
        for run in interval["runs"]:
            _apply_run_formatting(run, font_name, font_size, bold, italic, underline, strike)
...
def _set_paragraph_text(para: Any, text: str, font_name: str | None = None, font_size: float | None = None,
                        align: str | None = None, bold: bool | None = None, italic: bool | None = None,
                        underline: bool | None = None, strike: bool | None = None, bullet: bool | None = None) -> None:
    if _set_math_aware_paragraph_text(para, text, font_name, font_size, align, bold, italic, underline, strike, bullet):
        return
