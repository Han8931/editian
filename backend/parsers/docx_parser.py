...
def _has_ancestor(element, stop, tag: str) -> bool:
...
def _visible_text_from_element(element, stop) -> str:
    parts: list[str] = []
...
def _paragraph_text_with_math_placeholders(para) -> str:
    parts: list[str] = []
    equation_index = 0
    for child in para._p:
...
def _render_paragraph(para, index: int | None = None) -> str:
    ...
    inner = _render_paragraph_inner(para) or '&nbsp;'
    return f'<{tag}{style_attr}{data_attr}>{inner}</{tag}>'


def _render_equation_placeholder(index: int, display: bool) -> str:
    label = f'[Equation {index}]'
    style = (
        'display: block; margin: 0.35em 0; text-align: center; '
    ...
    return f'<span class="docx-equation-placeholder" style="{style}">{_html.escape(label)}</span>'


def _render_paragraph_inner(para) -> str:
    parts: list[str] = []
    equation_index = 0
    for child in para._p:
        if child.tag == qn('w:r'):
            parts.append(_render_run(Run(child, para)))
        elif child.tag == qn('w:hyperlink'):
            url = child.get('{http://schemas.openxmlformats.org/officeDocument/2006/relationships}id')
            for r in child.findall(qn('w:r')):
                parts.append(_render_run(_HyperlinkRun(r, para, url)))
        elif child.tag == qn('w:ins'):
            for r in child.findall(qn('w:r')):
                parts.append(_render_run(Run(r, para)))
        elif _is_math_element(child):
            equation_index += 1
            parts.append(_render_equation_placeholder(equation_index, child.tag == qn('m:oMathPara')))
    return ''.join(parts)
...
def parse_docx(file_path: str) -> dict[str, Any]:
    ...
    for i, para in enumerate(doc.paragraphs):
        text = _paragraph_text_with_math_placeholders(para)
        if text.strip():
            paragraphs.append({
                "index": i,
                "text": text,
    ...
    for child in doc.element.body:
        ...
        if tag_name == qn('w:p'):
            ...
            if list_info:
                ...
                para_text = _paragraph_text_with_math_placeholders(para)
                data_attr = f' data-para-index="{idx}"' if para_text.strip() else ''
                inner = _render_paragraph_inner(para) or '&nbsp;'
                ...
                close_all_lists()
                para_text = _paragraph_text_with_math_placeholders(para)
                index = idx if para_text.strip() else None
